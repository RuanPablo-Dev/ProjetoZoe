...
void generate_random_value(int data_type, char* buffer);
...
int main(int argc, char* argv[]) {
    if (argc < 8 || (argc - 7) % 2 != 0) {
        ...
        printf("Exemplo: %s 1 6 2023 30 6 2023 temp 0 umid 2 press 1\n", argv[0]);
        return 1;
    }
    ...
    for (int i = 0; i < sensor_count; i++) {
        ...
        sensors[i].data_type = atoi(argv[8 + i*2]);
        if (sensors[i].data_type < 0 || sensors[i].data_type > 3) {
